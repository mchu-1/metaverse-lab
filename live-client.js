...
export class LiveClient {
        ...
        tools: [
          {
            functionDeclarations: [
              ...
              },
              {
                name: "locate_object",
                description:
                  "Locates a specific object or feature in the environment using the map texture and returns its coordinates (x, y).",
                parameters: {
                  type: "OBJECT",
                  properties: {
                    query: {
                      type: "STRING",
                      description:
                        "The object to find (e.g. 'DNA helix', 'emergency exit').",
                    },
                  },
                  required: ["query"],
                },
              },
              {
                ...
                description:
                  "Captures a screenshot of the user's current view and provides a visual description of the scene.",
        ...
        try {
          if (name === "look_at") {
            ...
            if (coords.x !== undefined && coords.y !== undefined) {
              ...
              result = {
                error: "Object not found or vision error.",
                details: coords,
              };
            }
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (canvas) {
          ...
          console.log(
            `[LiveClient] Captured Screenshot. Data URL length: ${dataURL.length}`
          );
  ...
  async queryVisionModel(base64Image) {
    ...
    const prompt =
      "Describe this scene concisely. Identify key objects and the setting.";

    // Check for Proxy Usage
    // If apiKey looks like a URL, it's a Proxy URL
    if (
      this.apiKey &&
      (this.apiKey.startsWith("http") || this.apiKey.startsWith("wss"))
    ) {
      // Resolve Vision Endpoint from Proxy Base
      let proxyBase = this.apiKey;
      if (proxyBase.startsWith("wss:"))
        proxyBase = proxyBase.replace("wss:", "https:");
      if (proxyBase.startsWith("ws:"))
        proxyBase = proxyBase.replace("ws:", "http:");
